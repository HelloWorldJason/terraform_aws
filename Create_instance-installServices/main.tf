...
resource "aws_instance" "this" {
    ...
    //user_data = templatefile("set_nginx.sh",{time=timestamp()})      #读取同级别目录下的文件

    tags = {   # tags主要在EC2中的tag进行加一个tags标签控制台实例中name字段，方便查看读取维护.
...
data "aws_security_groups" "default" {     #data内置函数主要作用是查询功能，不做执行操作
    filter {
      name = "group-name"
      values = ["default"]
    }
    # filter主要做过滤，通过aws_security_groups接口获取到安全组的列表，通过values来获取我们要的default的值。
}
...
resource "aws_security_group_rule" "ssh" {
    ...
    security_group_id =  data.aws_security_groups.default.ids[1]    #ids[0]意思是拿第一个值
}
